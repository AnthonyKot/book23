...
on an earlier object (from ch. 10 on, one in-body back reference per chapter). What fixed it was a
running system with fixed numbers reused later (the pair from ch. 4 is *the* pair in ch. 7, 8, 9,
15; ch. 14 corrects ch. 13's measured noise). Rule for Book 23: **the service is the spine**. A
...
is what made it memorable. Every Book 23 exercise needs at least one safe-looking decoy and one
pair of near-identical handlers with opposite verdicts. The Book 11 critique (2026-09-22) asked
for exactly this: "add a safe-looking decoy handler".
...
code, the exercise tables are tests, `verify.sh` runs them in vulnerable and fixed modes and
fails if a vulnerable route *passes* its two-user test. A chapter whose code is not in
...
researcher's write-up or the vendor's disclosure is the evidence. Never invent a detail of a real
incident; if the record is silent, the prose is silent.

**L11. Lengths hold when set in advance.** Book 2's ±15% rule kept the rework from ballooning
(largest growth +14%). Chapter target 1,600–2,100 words; the pilot fixes the number; later
chapters stay within ±15% of the pilot's length.
...
2. Diff against the incident's primary source: every number present there? (L10)
3. Read the exercise answers: does each name the ID's origin and the comparison? (L5)
4. Is there a safe-looking decoy and a near-identical pair with opposite verdicts? (L8)
5. Does the chapter reuse a named earlier object in its worked example? (L1)
6. Does `verify.sh` show the chapter's vulnerable tests failing and fixed tests passing? (L9)
7. Word count within ±15% of the pilot? (L11)
...
book23/
  ...
  verify.sh          links, structure, tests in both modes
```
