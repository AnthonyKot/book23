...
| 11 | `chapter11RequestPath` composes the final fixed build in one function: mux → ch. 4 lookup budget → ch. 5 authorization (ch. 1 scope, then role) → ch. 8 error writer → ch. 2 identity → ch. 9 host filter; `TestChapter11EveryRouteEveryCaller` walks all 20 routes × {anonymous, Alice, Ben, Dana} asserting no cross-tenant data, no internal fields, opaque `GET /v2/invoices/{id}` 404s, and declared status (two named 400 fixtures: OTP verify without a challenge, approval of an open quote); exercise A–H asserted |

The table is complete for the drafted book.
